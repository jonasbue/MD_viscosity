...
def simplified_enskog(F, pf, T, omega, sigma):
...
def fit_viscosity(filename, F, resolution, fit_sigma=True, pf_max = 0.2, xmin=0.01, xmax=0.51, to_original_file=False):
    ...
    # If true, returned parameters will fit the original data file.
    # If false, higher resolution data will be created instead.
    if to_original_file:
        pf_save = np.unique(pf_original)
    ...
    for i in range(len(T)):
        def f(pf, omega, sigma):
            return simplified_enskog(F, pf, T[i], omega, sigma)
        def g(pf, omega):
            return simplified_enskog(F, pf, T[i], omega, 1.0)
        if fit_sigma:
            ...
            #print(f"eta={eta[i]}")
            eta[i,:] = simplified_enskog(F, pf_save, T[i], collision_integral, sigma_eff)
            ...
            omega[i,:] = collision_integral
            eta[i,:] = simplified_enskog(F, pf_save, T[i], collision_integral, 1.0)
